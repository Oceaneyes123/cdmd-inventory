...
function updateProduct(productId, name, category, stock, costPrice, retailPrice) {
  try {
    db.prepare('UPDATE products SET name = ?, category = ?, stock = ?, cost_price = ?, retail_price = ? WHERE product_id = ?').run(name, category, stock, costPrice, retailPrice, productId);
  } catch (err) {
    console.error(err);
  }
}
...
app.put('/products/:productId', (req, res) => {
  ...
  console.log({productId, name, category, stock, costPrice, retailPrice})
